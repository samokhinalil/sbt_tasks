...
public class BasicsLesson {
	...
	public String littleKid(String word) {
		String result = word.replaceAll("Р", "Л");
		return result;
	}
	...
	public Object newClass() {
		Object o = new LethalWeapon("red", 2.0, 3);
		return o;
	}
	...
	public int stringExercise2(String input) {
		...
		for (char c : input.trim().toCharArray()) {
			if (c == ',') commaCount++;
			else if (c == '-') hyphenCount++;
			else if (c == ' ') spaceCount++;
		}

		return length * commaCount * hyphenCount * spaceCount * input.indexOf("R") * input.lastIndexOf("L");
	}
	...
	public String stringExercise3(String input) {
		...
		if (wordsCount == 2) {
			if (input.contains("Hello")) {
				...
				result = (strings[1] + " " + strings[0]).toLowerCase();
			}
		}
